...
export function registerApiServer(
  ...
  const routes: ApiRoute[] = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        engine.addRule({
          ...
          ...(typeof body.aggregation_window_ms === 'number' ? { aggregationWindowMs: body.aggregation_window_ms } : {}),
          ...(body.mode === 'goal' || body.mode === 'action' ? { mode: body.mode as 'action' | 'goal' } : {}),
          ...(typeof body.goal === 'string' && body.goal ? { goal: body.goal } : {}),
    ...
    },
    {
      method: 'POST',
      path: 'rules/approve',
      handler: async (req, res, _q) => {
        const body = await requireJson(req, res)
        if (!body) return
        const { ruleId } = body
        if (!ruleId) {
          json(res, 400, { error: 'ruleId is required' })
          return
        }
        const ok = engine.approveRule(String(ruleId))
        if (!ok) {
          json(res, 404, { error: `rule not found: ${ruleId}` })
          return
        }
