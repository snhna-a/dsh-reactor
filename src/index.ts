...
import { registerReactorSkill } from './skill.js'
 ...
 */
// Hard dependencies: tools/webServer for the core engine + API + widget;
// agents/agentDefaultModel/sessions are required for goal-mode Agent Sessions
// (mirrors dsh-cron). agentPresets/sessionTitle/permissionPresets stay
// optional and are accessed via ctx.get in goal-session.ts.
export const inject = ['tools', 'webServer', 'agents', 'agentDefaultModel', 'sessions']
...
export function apply(ctx: Context, config: ReactorConfig): void {
  ...
  // P1: register the conversational skill so the Agent knows how to create
  // rules from natural language (event source / condition / goal inference).
  registerReactorSkill(ctx)
