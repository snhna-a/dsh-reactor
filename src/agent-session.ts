...
export async function runAgentSession(
  ...
  try {
    ...
    let tokens: { inputTokens: number; outputTokens: number; cachedTokens: number; reasoningTokens: number; totalTokens: number } | undefined
    try {
      ...
      if (usageVal && typeof usageVal === 'object') {
        ...
        if (input !== undefined || output !== undefined) {
          tokens = {
            ...
            cachedTokens: 0,
            reasoningTokens: 0,
            totalTokens: (typeof input === 'number' ? input : 0) + (typeof output === 'number' ? output : 0),
          }
        }
      }
    } catch {
