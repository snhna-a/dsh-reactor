...
export function registerTools(ctx: Context, engine: ReactorEngine): void {
    ...
    defineTool({
      ...
      parameters: {
        ...
        cooldown_ms: {
          type: 'number',
          description:
            'Cooldown in milliseconds after a trigger before the rule can fire again. Default 0 (no cooldown). Recommended 300000 (5 min) for polling sources to avoid rapid re-trigger.',
        },
        mode: {
          type: 'string',
          enum: ['action', 'goal'],
          description:
            'Execution mode. "action" (default): run preset shell/webhook/agent-talk actions. ' +
            '"goal": spawn an isolated autonomous Agent Session that ReActs to achieve the goal — preferred for non-trivial tasks like "pull code and run tests".',
        },
        goal: {
          type: 'string',
          description:
            'Natural-language goal for goal mode. The Agent receives the event payload + this goal and autonomously figures out how to do it. ' +
            'Example: "Clone the repo from the release URL, install dependencies, run tests, and report the result."',
        },
        constraints: {
          type: 'object',
          additionalProperties: true,
          description:
            'Optional constraints for goal mode: { workingDir, maxSteps (default 20), timeoutMinutes (default 30), allowedTools[] }.',
        },
      },
      output: {
        schema: {
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute() {
        // v0.12.3: lossless JSON snapshot — strip non-serializable runtime refs
        // so the tool result doesn't fail the host's lossless-JSON check.
        const raw = engine.listRules()
  ...
  ctx.tools.register(
    defineTool({
      ...
      async execute(args: any) {
        ...
        return JSON.parse(JSON.stringify(records.slice(-limit))) as unknown as any[]
      },
    }),
  )
}
