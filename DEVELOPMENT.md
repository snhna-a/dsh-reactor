...
3. 产物格式必须为 `window.__ModuleLoader__.load({ id, factory: (require) => {...} })`（esbuild bundle + IIFE 包装，见 `scripts/build-client.mjs`）
...
5. React 经 `__require("react")` 从宿主模块表解析（esbuild external），无需自带 React
...
| 文件 | 变更 |
|------|------|
| `src/client/Widget.tsx` | 新增：挂件全部 UI（约 32KB） |
| `src/client/api.ts` | 新增：REST 客户端（ReactorApi，覆盖 7 路由） |
| `src/client/index.ts` | 新增：client 入口（slots 注册） |
| `scripts/build-client.mjs` | 新增：esbuild + ModuleLoader 包装构建脚本 |
| `lib/client.js` + `.map` | 构建产物（约 42KB / 61.5KB） |
...
| 浏览器渲染 | ✅ 已目验：headless Edge 加载 dsh web，`--dump-dom` 确认 `.rxw-root`（fixed 右下角）+ mascot `<img>` + 全量 rxw-* 面板节点渲染；整页截图右下角可见浅蓝 mascot（主题角色旁） |
...
## 十、四期升级功能（v0.4.0）——浅色主题 + 时钟主题形象 + 交互重构

> 本节为四期实现完成后的增量能力记录。核心目标：把挂件从"深色毛玻璃 + 圆形裁剪形象"升级为**浅色主题 + 时钟主题全身立绘 + 悬浮按钮展开面板**的交互模式，配色与新形象（深蓝星空制服 + 金色眼睛 + 齿轮/怀表元素）深度搭配。

### 10.1 升级总览

| 四期目标 | 实现方式 | 模块 | 状态 |
|----------|----------|------|------|
| 形象更换 | 时钟主题 Q 版角色（深蓝星空制服 + 金色眼睛 + 齿轮/怀表/沙漏），透明背景全身立绘，非圆形裁剪 | `docs/assets/reactor-mascot.png` + `/reactor/mascot.png` 路由 | ✅ 已实现 |
| 交互重构 | 点击形象只显示统计气泡（不展开面板）；悬浮形象时显示"⚙ 管理面板"按钮，点击按钮才展开面板 | `src/client/Widget.tsx` | ✅ 已实现 |
| 关闭按钮移位 | 从面板右上角（✕ 角落按钮）移至面板底部中央，做成"⏻ 收起面板"胶囊按钮 | `src/client/Widget.tsx` | ✅ 已实现 |
| 浅色主题 | 白色面板背景 + 深蓝渐变标题栏（#1e3a5f→#3b4f7a）+ 金色强调色（#f0c040）+ 浅灰卡片（#f8fafc） | `src/client/Widget.tsx` CSS | ✅ 已实现 |
| 扩展设置 | 挂件大小（0.5–2.5x）、面板宽度（360–640px）、形象 URL、气泡开关，全部 localStorage 持久化 | `src/client/Widget.tsx` | ✅ 已实现 |
| 形象透明显示 | `object-fit: contain` + `drop-shadow` 投影，保留全身立绘完整轮廓，不再圆形裁剪 | `src/client/Widget.tsx` CSS | ✅ 已实现 |
...
| 交互 | v0.3（旧） | v0.4（新） |
|------|-----------|-----------|
| 点击形象 | 直接展开/收起管理面板 | 只显示统计气泡（触发次数、规则数、失败数），不展开面板 |
| 展开面板 | 点击形象 | 悬浮形象 → 顶部出现"⚙ 管理面板"胶囊按钮 → 点击按钮展开 |
| 关闭面板 | 面板右上角 ✕ 角落按钮 | 面板底部中央"⏻ 收起面板"胶囊按钮（深蓝底 + 金字） |
| 形象显示 | 圆形裁剪（72px 圆） | 透明全身立绘（96px 基准，可缩放），drop-shadow 投影 |
| 面板主题 | 深色毛玻璃（rgba(15,23,42,.92) + blur） | 浅色（白色面板 + 深蓝标题栏 + 金色强调） |

### 10.3 配色系统（与时钟主题形象搭配）

| 用途 | 色值 | 说明 |
|------|------|------|
| 标题栏主色 | `#1e3a5f` → `#3b4f7a` | 深蓝渐变，匹配形象制服主色 |
| 强调色 | `#f0c040` / `#d4a843` | 金色，匹配形象眼睛/齿轮/怀表 |
| 面板背景 | `#ffffff` | 纯白，浅色主题 |
| 卡片背景 | `#f8fafc` | 浅灰，与白色面板形成层次 |
| 边框 | `#e2e8f0` | 浅灰边框 |
| 主文字 | `#1e293b` | 深灰 |
| 次要文字 | `#64748b` / `#94a3b8` | 中灰/浅灰 |
| 成功 | `#16a34a` | 绿色 |
| 失败 | `#dc2626` | 红色 |
| 导航激活 | 底部 `#f0c040` 金色下划线 | 与标题栏金色分隔线呼应 |

### 10.4 新增设置项

| 设置项 | 范围 | 默认值 | 存储键 |
|--------|------|--------|--------|
| 挂件大小 | 0.5x – 2.5x | 1.0x | `dsh-reactor:scale` |
| 面板宽度 | 360 – 640 px | 460 px | `dsh-reactor:panelW` |
| 形象图片 URL | 任意 URL | `/reactor/mascot.png` | `dsh-reactor:avatar` |
| 触发时自动弹气泡 | on/off | on | `dsh-reactor:bubble` |

设置入口：悬浮形象 → 左下角汉堡菜单按钮（☰）→ 弹出设置面板。

### 10.5 四期验证记录

| 验证项 | 结果 |
|--------|------|
| `tsc` 类型检查（host + client） | ✅ 0 错误 |
| client 构建（esbuild + ModuleLoader 包装） | ✅ lib/client.js 54414 字节（52.7KB） |
| 新形象复制到项目 | ✅ docs/assets/reactor-mascot.png 4117935 字节（4.1MB 高清透明 PNG） |
| dsh web 启动 | ✅ 端口 3080 监听，插件加载无崩溃 |
| mascot 路由 | ✅ HTTP 200，Content-Type: image/png，4.1MB 新形象 |
| REST API stats | ✅ HTTP 200，返回正确统计数据 |
| 构建产物包含新 UI 标记 | ✅ rxw-open-btn / rxw-close-btn / 管理面板 / 收起面板 / #1e3a5f / #f0c040 均存在 |

### 10.6 浏览器验证方法

1. 确保 dsh web 运行中（`npx @deepseek-ai/dsh web`）
2. 浏览器打开 `http://127.0.0.1:3080/`（用启动日志中的完整 URL，含 token）
3. **Ctrl+Shift+R 硬刷新**（清除旧 client 缓存）
4. 右下角应出现时钟主题角色形象（深蓝制服 + 金色眼睛）
5. **悬浮形象**：顶部出现"⚙ 管理面板"金色胶囊按钮，左下角出现汉堡菜单按钮
6. **点击形象**：弹出白色统计气泡（触发次数、规则数），不展开面板
7. **点击"⚙ 管理面板"按钮**：展开浅色管理面板（白色背景 + 深蓝标题栏 + 金色导航下划线）
8. **面板底部**："⏻ 收起面板"按钮，点击关闭面板
9. **汉堡菜单**：可调节挂件大小、面板宽度、更换形象 URL、开关气泡

### 10.7 四期遗留与后续建议

- 形象图片 4.1MB 较大，首次加载可能有短暂延迟；后续可生成压缩版（WebP 或 1024px 缩放版）
- 面板宽度调节后需重新打开面板生效（当前为即时生效，但位置计算基于旧宽度）
- 后续可选：深色/浅色主题切换、面板透明度调节、触发音效、形象多套皮肤切换
